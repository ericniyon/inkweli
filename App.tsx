...
const App: React.FC<AppProps> = ({ initialArticles = [], initialWriters }) => {
  ...
  if (showLandingFeed) {
    return (
      <div className="min-h-screen flex flex-col bg-white animate-fade-in">
        <SiteHeader
          variant="landing"
          searchValue={landingSearchQuery}
        ...
        <div className="flex-1 animate-fade-up">
          <LandingPageView
            ...
            onSearchQueryChange={setLandingSearchQuery}
            activeCategory={landingCategory}
            onActiveCategoryChange={setLandingCategory}
          />
        </div>
        <Footer />
      </div>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col animate-fade-in">
      ...
      {/* App sub-nav: Home, Library, Profile, etc. */}
      <div className="sticky top-14 sm:top-16 z-[90] bg-white border-b border-slate-100 px-4 sm:px-6 py-2 flex items-center gap-2 overflow-x-auto no-scrollbar">
        ...
        <div className="md:hidden relative flex-1 flex justify-center">
          <button
          ...
          {isMobileNavOpen && (
            <>
              <div className="fixed inset-0 z-[89]" aria-hidden onClick={() => setIsMobileNavOpen(false)} />
              <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 py-2 w-40 bg-white border border-slate-200 rounded-xl shadow-lg z-[91]">
                {(['FEED', 'LIBRARY', 'PROFILE', 'STORIES', 'STATS'] as const).map((view) => (
                  <button
                    key={view}
                    onClick={() => { handleNavClick(view); setIsMobileNavOpen(false); }}
      ...
      <div className="flex flex-1 flex-col sm:flex-row min-h-0" onClick={() => setIsMobileNavOpen(false)}>
        <aside className="hidden sm:flex flex-col border-r border-slate-200 w-48 min-w-[12rem] flex-shrink-0 sticky top-24 left-0 self-stretch h-[calc(100vh-6rem)] overflow-y-auto no-scrollbar py-5 bg-slate-50 z-10">
          <div className="px-3">
        ...
        {/* Main Content Area */}
        <main className="flex-1 flex flex-col lg:flex-row max-w-[1600px] mx-auto w-full animate-fade-up">
