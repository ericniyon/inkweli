    ...
    <div className="fixed inset-0 z-[200] flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        <header className="px-6 py-6 border-b border-slate-100 flex justify-between items-center">
          <h2 className="text-xl font-black text-slate-900">Responses ({responses.length})</h2>
        ...
        <div className="p-6 border-b border-slate-100">
          <div className="bg-slate-50 rounded-2xl p-4 shadow-inner">
            ...
            <textarea 
              ...
              className="w-full bg-transparent border-none text-sm focus:ring-0 outline-none resize-none min-h-[100px] font-medium Charter"
              value={newText}
              onChange={(e) => setNewText(e.target.value)}
            />
            <div className="flex justify-end gap-3 pt-4">
              <button onClick={() => setNewText('')} className="text-xs font-bold text-slate-500 hover:text-slate-900">Cancel</button>
              <button 
                onClick={handlePost}
                disabled={!newText.trim()}
                className="bg-emerald-600 text-white px-4 py-1.5 rounded-full text-xs font-bold hover:bg-emerald-700 transition disabled:opacity-50"
              >
                Respond
              </button>
            </div>
        ...
        <div className="flex-grow overflow-y-auto px-6 py-8 space-y-8 no-scrollbar">
          {responses.map(resp => (
            <div key={resp.id} className="space-y-4">
              ...
              <p className="text-sm text-slate-700 leading-relaxed font-medium Charter">{resp.text}</p>
              <div className="flex items-center gap-4">
                <button className="flex items-center gap-1.5 text-slate-400 hover:text-slate-900 transition">
                   ...
                   <span className="text-[10px] font-bold">{resp.claps || 0}</span>
                </button>
                <button className="text-[10px] font-bold text-slate-400 hover:text-slate-900">Reply</button>
              </div>
            </div>
          ))}
          {responses.length === 0 && (
            <div className="text-center py-20 text-slate-400 italic text-sm">Be the first to respond!</div>
          )}
        </div>
      </div>
    </div>
